...
def init_test_data():
    # add item1
    item = CharityItem(u"富士山寄付", u"富士山の登山道を改善する", u"富士山の登山道を改善する", "1.png", "11.png", 3, 12, 1, 2000, 1700)
    dictionary = item.to_dict()
    redis_manager.set_dict(dictionary, item.major, item.minor)

    # add item2
    item = CharityItem(u"水族館存続のために！", u"水族館存続のために！", u"水族館存続のために！", "2.png", "12.png", 2, 12, 2, 2500, 1700)
    dictionary = item.to_dict()
    redis_manager.set_dict(dictionary, item.major, item.minor)

    # add item3
    item = CharityItem(u"古民家の再活用のために！", u"古民家の再活用のために！", u"古民家の再活用のために！", "3.png", "13.png", 1, 12, 3, 2000, 1500)
    dictionary = item.to_dict()
    redis_manager.set_dict(dictionary, item.major, item.minor)

    # add item4
    item = CharityItem(u"美しい山を守る！", u"ゴミを無くして！", u"ゴミを無くして美しい山を取り戻したい！", "4.png", "14.png", 4, 12, 4, 2000, 1300)
    dictionary = item.to_dict()
    redis_manager.set_dict(dictionary, item.major, item.minor)

    # add item5
    item = CharityItem(u"お酒作りの美味しい水を守りたい！", u"お酒作りの美味しい水を守りたい！", u"お酒作りの美味しい水を守りたい！", "5.png", "15.png", 2, 12, 5, 2000, 1100)
    dictionary = item.to_dict()
    redis_manager.set_dict(dictionary, item.major, item.minor)
